...
df = 3
list(dim,cov.param, cov.naive, cov.vfcp,cov.nested,len.param, len.naive, len.vfcp,len.nested) = four_methods_comparison(nrep=100, l=60, lambda_seq=seq(0,200,l=100), dim=round(seq(5,300,l=l)), n=200, n0=100, rho=0.5, alpha=0.1, setting="nonlinear", df=3)
...
df = 5
list(dim,cov.param, cov.naive, cov.vfcp,cov.nested,len.param, len.naive, len.vfcp,len.nested) = four_methods_comparison(nrep=100, l=60, lambda_seq=seq(0,200,l=100), dim=round(seq(5,300,l=l)), n=200, n0=100, rho=0.5, alpha=0.1, setting="nonlinear", df=5)
len.param = len.param/len.vfcp
len.naive = len.naive/len.vfcp
len.nested = len.nested/len.vfcp
len.vfcp = len.vfcp/len.vfcp
...
data_nonlinear_fm=force_bind(a3_cov,a5_cov,a3_len,a5_len)
colnames(data_nonlinear_fm) <- c("V1","V2","sd","Method")
save(data_nonlinear_fm,file="non_linear_data_to_plot.RData")